...
GenCost = namedtuple('GenCost', ['startup', 'shutdown', 'pwlc'])
...
def create_cost_data_from_rts_data(rtsdata):
    # returns named tuple with generator cost information
    gencost = []
    pwlcs = create_pwlcost_from_rts_data(rtsdata)
    for i,g in enumerate(rtsdata.gen):
        su_cost = g['non_fuel_start_cost_dollar']
        su_cost += g['start_heat_warm_mbtu'] + g['fuel_price_dollar_per_mmbtu']
        sd_cost = g['non_fuel_shutdown_cost_dollar']
        gencost.append(
            GenCost(su_cost, sd_cost, pwlcs[i])
        )
    return gencost

def prepare_da_timeseries_data(first_period, horizon, rtsdata, psdata):

    gen_da_series = ['pv_da', 'wind_da', 'hydro_da', 'rtpv_da']
    
    gen_da_ts_dict = {}
    for ts_key in gen_da_series:
        ts_df = rtsdata.timeseries[ts_key]
        ts_df = ts_df.query('Year == @first_period[0] and Month == @first_period[1] and Day == @first_period[2] and Period >= @first_period[3] and Period <= @first_period[3]+@horizon')
        ts_df = ts_df.drop(['Year', 'Month', 'Day', 'Period', 'Timestamp'], axis=1)
        gen_da_ts_dict.update(ts_df.to_dict('list'))
    
    load_da_ts_dict = {}
    ts_df = rtsdata.timeseries['load_da_regional']
    ts_df = ts_df.query('Year == @first_period[0] and Month == @first_period[1] and Day == @first_period[2] and Period >= @first_period[3] and Period <= @first_period[3]+@horizon')
    ts_df = ts_df.drop(['Year', 'Month', 'Day', 'Period', 'Timestamp'], axis=1)
    load_da_ts = ts_df.to_dict('list')
    for bus in psdata.busdata:
        area_load = np.array(load_da_ts[f"{bus['area']}"])
        bus_load = area_load * bus['area_load_share']
        load_da_ts_dict.update({bus['id']: bus_load})

    return gen_da_ts_dict, load_da_ts_dict


def prepare_rt_timeseries_data(first_period, horizon, rtsdata, psdata):

    gen_rt_series = ['pv_rt', 'wind_rt'] 
    
    gen_rt_ts_dict = {}
    for ts_key in gen_rt_series:
        ts_df = rtsdata.timeseries[ts_key]
        ts_df = ts_df.query('Year == @first_period[0] and Month == @first_period[1] and Day == @first_period[2] and Period >= @first_period[3] and Period <= @first_period[3]+@horizon')
        ts_df = ts_df.drop(['Year', 'Month', 'Day', 'Period', 'Timestamp'], axis=1)
        gen_rt_ts_dict.update(ts_df.to_dict('list'))
    
    load_rt_ts_dict = {}
    ts_df = rtsdata.timeseries['load_rt_regional']
    ts_df = ts_df.query('Year == @first_period[0] and Month == @first_period[1] and Day == @first_period[2] and Period >= @first_period[3] and Period <= @first_period[3]+@horizon')
    ts_df = ts_df.drop(['Year', 'Month', 'Day', 'Period', 'Timestamp'], axis=1)
    load_rt_ts = ts_df.to_dict('list')
    for bus in psdata.busdata:
        area_load = np.array(load_rt_ts[f"{bus['area']}"])
        bus_load = area_load * bus['area_load_share']
        load_rt_ts_dict.update({bus['id']: bus_load})

    return gen_rt_ts_dict, load_rt_ts_dict
